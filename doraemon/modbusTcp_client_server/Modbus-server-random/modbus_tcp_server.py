...
def updating_writer(a):
    ...
    word_max_values = [
        ...
        0x0000, 0x0001, # float32 positive number min 1.4e-45
        0x7F7F, 0xFFFF, # float32 positive number max 3.4e+38
        0x8000, 0x0001, # float32 negative number min -3.4e+38
        0xFF7F, 0xFFFF, # float32 negative number max -1.4e-45
    ...
    length = int(DATA_LENGTH / 2)

    word_values = [random.randint(0, 20) for i in range(random_begin, length)]
    print(word_values)
    bit_values = [random.randint(0, 1) for i in range(length)]
    print(bit_values)
...
def main():
    ...
    store = ModbusSlaveContext(
        di=ModbusSequentialDataBlock(0, [0]*DATA_LENGTH),
        co=ModbusSequentialDataBlock(0, [0]*DATA_LENGTH),
        hr=ModbusSequentialDataBlock(0, [0]*DATA_LENGTH),
        ir=ModbusSequentialDataBlock(0, [0]*DATA_LENGTH))
